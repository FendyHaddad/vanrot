...
| docs and web | `apps/vanrot-site` learning surface | Demo-Capable through Phase 16C | Site documents implemented framework surface, supported commands, package references, diagnostics, conventions, examples, UI primitives, and maturity status with drift checks. | Phase 16C, Phase 24 | Phase 16C creates the learning site base from current data; Phase 24 performs the final exhaustive audit. |
| docs and web | `apps/vanrot-site` workspace app | Demo-Capable through Phase 16C | Site builds as a Vanrot app with router, pages, October tokens, `vanrotstyles.css`, a Phase 16B primitive gallery, and dedicated shadcn-style docs pages for every Phase 16B primitive. | Phase 16C, Phase 24 | Phase 24 remains the exhaustive final docs audit. |
...
| metadata | primitive registry metadata | Demo-Capable through Phase 16B | Registry lists supported primitives, generated file names, asset URLs, variants, native tags, docs paths, and future flavors. | Phase 9, Phase 16B | Current Phase 16B core primitives are button, card, badge, avatar, alert, loader, skeleton, and separator. |
...
| compiler | October UI tag diagnostics | Production-Ready for Phase 16A foundation | Supported `vr-*` primitives lower through metadata and unsupported `vr-*` UI tags use October diagnostics. | Phase 16A, Phase 16B | Additional layout, form, data, and overlay semantic tags remain sliced. |
| flavor | October UI | In Progress through Phase 16B | Production primitive catalog, variants, accessibility, tokens, and docs. | Phase 16 | Phase 16A establishes the foundation and Phase 16B adds core primitives; component coverage remains sliced. |
