...
| Area | Item | Current Maturity | Final TDD Expectation | Owner Phase | Notes |
|------|------|------------------|-----------------------|-------------|-------|
| route config | `defineRoutes(...)` | Production-Ready through 15C navigation decisions | Provides a typed route table with path, label, page/loadPage, route refs, parent normalization, redirects, guards, and diagnostics. | Phase 8, Phase 15A, Phase 15C | Route config remains the single source of truth; preloading stays in 15D. |
| route config | `createRoutes()` | Production-Ready | Authors route refs without parent-name string literals, supports child `.page(...)`, `.layout(...)`, and `.redirect(...)` builders, keeps page/layout/redirect targets separate at the TypeScript boundary, and exposes `redirectTo(...)` for structured params/query redirects. | Phase 15A, Phase 15B, Phase 15C | Object-form `defineRoutes(...)` remains compatible. |
...
| state | `provideRouter(...)` | Production-Ready through 15C navigation decisions | Sets route table, current match, route params, cleanup, initial redirect handling, and guarded initial navigation. | Phase 8, Phase 15C | Returns `Promise<boolean>` so async guard decisions can complete without a broad router event API. |
| state | `navigate(...)` | Production-Ready through 15C navigation decisions | Handles path updates, history behavior, params, query strings, invalid route behavior, redirects, blocking guards, and stale async navigation protection. | Phase 8, Phase 15A, Phase 15C | Guard/redirect navigation policy now runs through the shared decision pipeline. |
...
| DOM | `<vr route.name />` link | Production-Ready through 15C navigation decisions | Renders accessible anchors from route metadata without repeated path or label strings and routes normal clicks through guarded navigation. | Phase 8, Phase 15A, Phase 15C | Params, query, exact active state, and guard/redirect behavior are covered; custom labels stay deferred. |
