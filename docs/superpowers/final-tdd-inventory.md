...
| [x] | forms | `createFormController(...)` | Demo-Capable | Covers field registration, value/dirty/touched/errors, built-in validators, custom validators, reset, submit orchestration, and DOM control connection. | Phase 16E, Phase 21 | Phase 16E keeps the helper small; app code owns persistence and async save behavior. |
| [x] | data | `createTableController(...)` | Demo-Capable | Covers sorting, filtering, pagination, selection, loading, empty state, DOM filter wiring, and teardown behavior. | Phase 16E | Useful table state exists without becoming an enterprise grid or query/cache layer. |
| [x] | interaction | `createOverlayController(...)` | Demo-Capable | Covers open/close state, trigger/content registration, focus movement, focus restore, escape close, outside pointer close, inside pointer ignore, and disposal. | Phase 16F | Shared by dialog, drawer, and dropdown templates without adding a broad UI manager. |
| [x] | interaction | `createTabsController(...)` | Demo-Capable | Covers selected value state, trigger/panel registration, ARIA state, click selection, arrow-key movement, and disposal. | Phase 16F | Keeps tabs interaction reusable while templates own markup. |
| [x] | interaction | `createToastController(...)` | Demo-Capable | Covers accessible toast queue state, tone metadata, manual dismiss, timeout dismiss, and timer cleanup. | Phase 16F | App code owns persistence and notification policy. |
...
| [x] | template | static element parsing | Production-Ready | Covers nested elements, attributes, comments, unsupported syntax, slot outlets, and stable AST output. | Phase 3, Phase 12C | Parser now supports recursive control flow inside element content. |
...
| [x] | command | `vr create <name>` | Demo-Capable | Creates a routed Vanrot app, handles package managers, overwrite policy, workspace mode, and clean install. | Phase 5, Phase 14, Phase 26 | Generated app includes router by default. |
...
| [x] | command | `vr doctor` | Demo-Capable | Reports project health, Vanrot rule violations, missing scripts, raw template text, nested control-flow warnings, stable one-line findings, and structured result output. | Phase 5, Phase 14 | Production doctor still needs broader diagnostics, code frames, and docs links. |
...
| [x] | diagnostics | CLI diagnostic catalog | Demo-Capable | CLI-owned diagnostics keep stable codes for unknown commands, structured-output failures, command failures, and AI doorway errors. | Phase 14, Phase 26 | Final release should expand code coverage as more CLI commands become production-ready. |
...
| [x] | editor metadata | `@vanrot/router` `web-types.json` | Production-Ready | JetBrains Web Types metadata declares `<vr>`, `<vr-router>`, `<vr-outlet>`, and the `route.*` shorthand pattern so WebStorm and IntelliJ can recognize router templates without a full plugin. | Phase 27A | Published through the package `web-types` manifest field and covered by package tests. |
...
| [x] | editor metadata | `@vanrot/ui` `web-types.json` | Production-Ready | JetBrains Web Types metadata declares every UI component catalog selector, interaction anatomy selector, and key Vanrot dotted/plain attributes so IDEs can recognize UI templates without a full plugin. | Phase 27A | Published through the package `web-types` manifest field and checked against the UI catalog/registry in package tests. |
