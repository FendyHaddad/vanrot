...
| [x] | LSP | initialize and document sync | Demo-Capable | Server initializes over LSP stdio with stable server info, incremental document sync, and completion trigger support. | IntelliJ Plugin M0-M1, Phase 26 | Covered by `server.test.ts`, `lsp-handshake.test.ts`, and `completion-handler.test.ts`. |
