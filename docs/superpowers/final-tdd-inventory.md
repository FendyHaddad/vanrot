...
| [x] | docs | feature maturity ledger | Production-Ready | Completed phases, maturity rows, and plan checklists stay synchronized. | Phase 11 | Source of truth is `docs/superpowers/feature-maturity.md`. |
| [x] | docs | phase documentation verifier | Production-Ready | Verifier fails on mismatched completed phases, unchecked plan tasks, and stale maturity rows. | Phase 11 | Implemented by `scripts/verify-phase-docs.mjs`. |
...
| [x] | docs | temporary phase completion hook | Production-Ready | Hook blocks phase-completion commits that omit required tracker, plan, or inventory updates. | Phase 11, Phase 12A | Phase 12A adds inventory enforcement. |
...
| [x] | docs and web | site docs data registry | Demo-Capable through Phase 29 | Registry covers implemented framework docs, commands, package references, diagnostics, conventions, examples, UI primitives, formatter pipes, and maturity status. | Phase 16C, Phase 16D, Phase 16E, Phase 16F, Phase 24, Phase 29 | Component docs now consume rich Phase 16F registry metadata and Phase 29 formatter pipe metadata for package, diagnostic, example, generated-file, and route-reference coverage. |
...
| [x] | subpath imports | `@vanrot/behavior/<name>` | Production-Ready | Subpath export tests cover form, table, overlay, tabs, tooltip, toast, command-menu, positioned-layer, collapsible, selection, menu, toggle, scroll-area, portal, focus, calendar, drag-drop, table-resize, and all. | Phase 16H, Phase 28 | `@vanrot/behavior/all` has no hard cap because it is explicit opt-in. |
| [x] | forms | `createFormController(...)` | Demo-Capable | Covers field registration, value/dirty/touched/errors, built-in validators, custom validators, reset, submit orchestration, and DOM control connection. | Phase 16E, Phase 16H | Full production form state moved to `@vanrot/forms` in Phase 21. |
...
| [x] | interaction | Behavior Expansion suite | Production-Ready | Covers collapsible/accordion/disclosure, selection/listbox/select/combobox/multi-selection, menu/context-menu/menubar/navigation-menu, toggle/toolbars, scroll-area, portal, focus/visually-hidden, calendar/date-picker, drag/drop reorder, and table column resize helpers. | Phase 28 | Focused behavior tests cover every new controller family, package exports, config validation, CLI create selection, docs, and AI bundle references. |

## `@vanrot/forms`

| Tested | Area | Item | Current Maturity | Final TDD Expectation | Owner Phase | Notes |
|---|---|---|---|---|---|---|
| [x] | package boundary | first-party forms package | Production-Ready | Package export, build, typecheck, release metadata, publish list, docs, and runtime-size guardrails cover `@vanrot/forms`. | Phase 21 | Package uses runtime signals but does not add browser runtime weight. |
| [x] | field state | `field(...)` and `createForm(...)` | Production-Ready | Named refs, nested refs, value, initial value, dirty, touched, disabled, pending, validity, messages, reset, submit, schema adapters, metadata, and server-error hooks are covered. | Phase 21 | Messages stay quiet initially while validity remains truthful. |
| [x] | validation | built-in and custom validators | Production-Ready | `required`, `minLength`, `email`, custom validators, `createFormMessage`, message source metadata, touch/change reveal, and all-errors-on-submit are covered. | Phase 21 | Validators stay plain Vanrot functions; schema libraries are optional adapters, not core dependencies. |
| [x] | async resources | `createFormResource(...)` | Production-Ready | Loading, success, error, value, refresh, stale, cancellation, ignored stale results, latest-interaction-wins, and async validators are covered. | Phase 21 | Resources stay form-scoped; standalone resources remain deferred. |
| [x] | repeated fields | `fieldArray(...)` | Production-Ready | Add, remove, move, stable keys, nested item refs, path metadata, and validation are covered. | Phase 21 | Field-array rows keep app-owned markup and data policy. |
| [x] | wizard forms | `createWizard(...)` | Production-Ready | Step state, current step, completed steps, blocked steps, per-step validation, and submit gates are covered. | Phase 21 | Wizard helpers consume form state instead of creating a second form model. |
| [x] | server errors | `applyServerErrors(...)` | Production-Ready | Field-level, nested array path, form-level, resource, and local-validation coexistence are covered. | Phase 21 | Backend contracts stay app-owned. |
| [x] | draft persistence | `createDraftStorage(...)` | Production-Ready | Local, session, custom adapter, version mismatch, restore, save, clear, reset, and sensitive-field warning behavior are covered. | Phase 21 | Password, secret, token, key, and credential names are excluded unless explicitly allowed. |
| [x] | UI helpers | `formField(...)` | Production-Ready | Label, description, validation IDs, server message IDs, pending, disabled, array rows, and wizard steps are covered. | Phase 21 | Helpers produce metadata for `@vanrot/ui` without hiding business logic in templates. |
| [x] | metadata and diagnostics | `.form.ts` metadata and Vite diagnostics | Production-Ready | Form metadata, sensitive-field diagnostics, repeated string path terminal warnings, future Forge metadata shape, docs, and AI reference rows are covered. | Phase 21 | Vite consumes forms metadata without taking a package dependency on `@vanrot/forms`. |
| [x] | testing helpers | `createFormTest(...)` | Production-Ready | Setting values, touching fields, submitting forms, inspecting messages, async resources, draft persistence, and server errors are covered by forms tests. | Phase 21 | Helpers live in `@vanrot/forms`; no `@vanrot/testing` bridge shipped in Phase 21. |

## `@vanrot/formatters`

| Tested | Area | Item | Current Maturity | Final TDD Expectation | Owner Phase | Notes |
|---|---|---|---|---|---|---|
| [x] | package boundary | first-party formatters package | Production-Ready | Package export, build, typecheck, release metadata, publish list, docs, AI bundle, and runtime-size guardrails cover `@vanrot/formatters`. | Phase 29 | Package is used by generated output only when templates use pipes; it does not add browser runtime weight. |
| [x] | text | text formatter suite | Production-Ready | `uppercase`, `lowercase`, `titlecase`, `sentencecase`, `truncate`, `fallback`, `initials`, and display-string coercion are covered. | Phase 29 | Text formatting stays pure and synchronous. |
| [x] | date and time | date/time formatter suite | Production-Ready | `date`, `time`, `datetime`, `relativeTime`, `duration`, named date presets, and custom date pattern formatting are covered. | Phase 29 | Context supplies locale and timezone defaults. |
| [x] | number and money | number formatter suite | Production-Ready | `number`, `currency`, `percent`, `compact`, `filesize`, number presets, currency defaults, and fraction/separator options are covered. | Phase 29 | Context supplies locale and currency defaults. |
| [x] | lists and messages | list/count/form-message helpers | Production-Ready | `join`, `count`, `plural`, `message`, and `messages` handle empty values, arrays, and custom labels. | Phase 29 | Message formatting does not couple the package to `@vanrot/forms`. |
| [x] | masks | generic mask formatter | Production-Ready | `mask` and `maskPattern` cover placeholder replacement, unmatched input, and reusable country/business masks. | Phase 29 | Enterprise country-specific masks can be named presets instead of repeated string literals. |
| [x] | custom pipes | `definePipe(...)` and `enumPipe(...)` | Production-Ready | Custom synchronous pipes, enum-backed display labels, default labels, invalid definitions, and async rejection are covered. | Phase 29 | Domain enums can live in separate app files and be imported into `.pipe.ts` files. |
| [x] | registry | `createPipeRegistry(...)` and `applyVanrotPipeChain(...)` | Production-Ready | Built-ins, custom pipes, namespace presets, chaining, unknown names, invalid arguments, and registry diagnostics are covered. | Phase 29 | Compiler browser and server output call this helper path. |
| [x] | testing | `createPipeTest(...)` | Production-Ready | Focused custom pipe tests can invoke a pipe with controlled context without compiling a template. | Phase 29 | Public subpath `@vanrot/formatters/testing` is exported. |

## `@vanrot/seo`

| Tested | Area | Item | Current Maturity | Final TDD Expectation | Owner Phase | Notes |
|---|---|---|---|---|---|---|
| [x] | package API | SEO literal owners | Production-Ready | Robots directives, Open Graph types, Twitter cards, schema types, sitemap change frequencies, canonical policies, diagnostic codes, and package name stay stable for generated files and app utilities. | Phase 27 | Covered by `packages/seo/tests/constants.test.ts`; constants are the string source of truth. |
| [x] | package API | static and dynamic metadata | Production-Ready | `defineSeo`, `defineDynamicSeo`, `isDynamicSeo`, and `resolveSeoInput` support static and async metadata without importing SEO into runtime. | Phase 27 | Dynamic metadata is opt-in at the app layer. |
| [x] | package API | metadata ladder and canonical resolution | Production-Ready | Global config, route overrides, page overrides, and dynamic/async SEO resolve in order, with canonical URL handling that allows missing production `siteUrl` until launch. | Phase 27 | Site docs explain this ladder explicitly. |
| [x] | package API | structured data and social helpers | Production-Ready | JSON-LD helpers, raw schema payloads, Open Graph/Twitter helpers, and social image validation are typed and tested. | Phase 27 | Social images are validated only; the package does not create preview artwork. |
| [x] | package API | sitemap, robots, render, and head helpers | Production-Ready | Sitemap XML, robots.txt, SSR-safe tag rendering, and explicit client head updates are covered with focused tests. | Phase 27 | Build output stays outside `@vanrot/runtime`. |
| [x] | diagnostics | SEO diagnostics | Production-Ready | Missing title/description, missing or invalid `siteUrl`, invalid social images, and strict-mode escalation produce stable package diagnostics. | Phase 27 | Config and CLI doctor consume related diagnostics at the app level. |
...
| [x] | template | template pipes in interpolation | Production-Ready | Covers pipe parsing, arguments, namespace variants, chaining, source spans, diagnostics, browser lowering, server lowering, and generated imports. | Phase 29 | Generated output imports `@vanrot/formatters` only for templates that use pipes. |
...
| [x] | diagnostics | `VR_PIPE_*` template pipe diagnostics | Production-Ready | Unknown names, unknown variants, duplicate names, duplicate presets, invalid arguments, invalid definitions, async handlers, source line/column, code frames, suggestions, and docs paths are covered. | Phase 29 | Vite forwards the same diagnostics into terminal output during startup and rebuild. |
...
| [x] | output | readable generated output | Production-Ready | Generated output stays stable, inspectable, and source-map aligned. | Phase 3, Phase 12C, Phase 14 | Command-facing project inspection now belongs to `vr doctor --inspect`; generated-output source-map explanation remains covered by compiler tests and docs. |
...
| [x] | behavior config | `behavior.enabled` | Production-Ready | Normalizes the optional behavior list and rejects unknown behavior helper names with stable config diagnostics across the Phase 16H and Phase 28 helper names. | Phase 16H, Phase 28 | Generated apps set this only when `vr create --behavior` opts in. |
| [x] | seo config | `seo` domain | Production-Ready | Normalizes the optional SEO domain without opt-out residue, keeps `siteUrl` optional, validates syntax even when `siteUrl` is missing, and exposes editor helpers for installer upserts. | Phase 27 | Adds `VRCFG014` through `VRCFG020`; missing `siteUrl` is a warning, malformed syntax is an error. |
| [x] | formatting config | `formatting` domain | Production-Ready | Normalizes locale, timezone, and currency defaults, validates empty values, and feeds compiler pipe context through Vite. | Phase 29 | Defaults are `en-US`, `UTC`, and `USD`; diagnostics use `VRCFG_FORMATTING_*` codes. |
...
| Tested | Area | Item | Current Maturity | Final TDD Expectation | Owner Phase | Notes |
|---|---|---|---|---|---|---|
| [x] | API | template-file rule | Demo-Capable | Canonical `isVanrotTemplateFile(...)` keeps Vanrot authored templates distinct from entry, devtools, landing, and standalone design HTML across package tests, emitted artifacts, and plugin mirrors. | IntelliJ Plugin M0, Phase 26 | Source of truth is `packages/language-server/src/template-files.ts`; `dist/template-globs.json` feeds the IntelliJ plugin. |
...
| [x] | pipe discovery | `.pipe.ts` metadata and diagnostics | Production-Ready | Vite discovers custom pipe role files, reports duplicate/async diagnostics, watches pipe sources, and passes registry/context options into the compiler. | Phase 29 | Terminal diagnostics include file path, line, column, stable code, code frame, suggestion, and `/docs/formatters`. |
| [x] | build output | SEO sitemap and robots artifacts | Production-Ready | Vite emits `sitemap.xml` and `robots.txt` when `seo.siteUrl` is configured and skips output while the production URL is unknown. | Phase 27 | Depends on `@vanrot/seo` helpers without importing SEO into runtime. |
...
| [x] | command | `vr create <name>` | Demo-Capable through Phase 28 | Creates a routed Vanrot app, handles package managers, overwrite policy, workspace mode, clean install, optional behavior helper selection, expanded behavior names, and optional SEO selection. | Phase 5, Phase 14, Phase 16H, Phase 26, Phase 27, Phase 28 | Generated app includes router by default; `--behavior` and `--seo` opt into optional packages while `--no-behavior` and `--no-seo` keep the starter minimal. |
...
| [x] | command | `vr add seo` | Production-Ready | Installs `@vanrot/seo`, upserts the rich `seo` block in `vanrot.config.ts`, writes `src/app/seo.ts`, accepts optional `--site-url`, preserves user-owned config, and suggests `vr doctor`. | Phase 27 | Supports later opt-in after `vr create --no-seo`. |
...
| [x] | command | `vr remove behavior` | Production-Ready | Removes selected behavior helpers from `behavior.enabled`, prunes `@vanrot/behavior` when no helpers remain, and preserves package manager ownership across the expanded helper catalog. | Phase 16H, Phase 28 | Supports `--behavior`, `--all`, and `--package`; repeated removal is idempotent. |
...
| [x] | command | `vr doctor` / `vr doctor --inspect` | Demo-Capable through Phase 27 | Reports project health, Vanrot rule violations, missing scripts, raw template text, nested control-flow warnings, behavior package drift, SEO package/config drift, stable one-line findings, structured result output, and read-only project intelligence summaries. | Phase 5, Phase 14, Phase 16H, Phase 23, Phase 27 | Behavior checks flag unused installs and stale imports; SEO checks flag unused installs, config-without-package, imports-without-package, and config syntax diagnostics; `--inspect` replaces standalone `vr inspect`. |
...
| [x] | command | `vr cache clean` | Demo-Capable | Safely cleans allowlisted Vanrot-owned generated metadata with dry-run output. | Phase 18, CLI maintenance | Uses `clean`, not `clear`; current cleanup boundary is `.vanrot/cache` and `.vanrot/project-map.json`. |
...
| [x] | store | Inspection protocol | Production-Ready | `inspectStore(store)` exposes bounded `vanrot.store.inspection.v1` event history, observer subscription, unsubscribe, observer error isolation, and disabled inspector behavior. | Phase 20 | Covered by `packages/store/tests/inspection.test.ts` and `packages/store/tests/package.test.ts`. |
| [x] | store | Snapshots and replay | Production-Ready | Inspection can capture manual and automatic snapshots, replay recorded actions through the store reducer, report changed keys, and fail cleanly for missing snapshots. | Phase 20 | Covered by `packages/store/tests/replay.test.ts` and store inspection tests. |
| [x] | store | Effect lifecycle inspection | Production-Ready | Effects emit inspection events for start, success, retry, timeout, failure, cancellation, stale-write prevention, trace names, concurrency keys, and cancellation reasons. | Phase 20 | Covered by `packages/store/tests/effect-inspection.test.ts` and `packages/store/tests/effects.test.ts`. |
| [x] | docs | Store docs IA | Production-Ready | `/docs/store`, `/docs/store/inspection`, `/docs/store/replay`, and Store child pages exist as real routes, sidebar entries, and AI docs entries. | Phase 19, Phase 20 | Covered by site data, site pages, site docs, and AI docs verifiers. |
...
| Store | store | Production-Ready foundation and hardening | Red/green tests cover signal-native state, fluent actions, reducers, selectors, full effects, headless inspection, snapshots, replay, effect lifecycle events, docs, AI docs, examples, and size. Devtools Store panel UI remains future pipeline work in `@vanrot/devtools`. | Phase 19, Phase 20 |
