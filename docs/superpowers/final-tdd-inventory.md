...
| docs and web | `apps/vanrot-site` learning surface | Demo-Capable through Phase 16D | Site documents implemented framework surface, supported commands, package references, diagnostics, conventions, examples, UI primitives, and maturity status with drift checks. | Phase 16C, Phase 16D, Phase 24 | Phase 16D adds alphabetical component navigation, dogfoods shell primitives, and documents layout/navigation/media primitives; Phase 24 performs the final exhaustive audit. |
| docs and web | `apps/vanrot-site` workspace app | Demo-Capable through Phase 16D | Site builds as a Vanrot app with router, pages, October tokens, `vanrotstyles.css`, a Phase 16B primitive gallery, and dedicated shadcn-style docs pages for every Phase 16B and 16D primitive. | Phase 16C, Phase 16D, Phase 24 | Phase 24 remains the exhaustive final docs audit. |
| docs and web | site docs data registry | Demo-Capable through Phase 16D | Registry covers implemented framework docs, commands, package references, diagnostics, conventions, examples, UI primitives, and maturity status. | Phase 16C, Phase 16D, Phase 24 | Component docs now derive alphabetical sidebar links from the current UI primitive docs data. |
...
| command | `vr add <Phase 16B primitive>` | Demo-Capable | Adds app-owned files for button, card, badge, avatar, alert, loader, skeleton, and separator without overwriting user files. | Phase 9, Phase 16B | Respects tokens, `ui.styles`, starter usage patching, and registry-driven supported primitive output. |
| command | `vr add <local-prefix> <Phase 16B primitive>` | Demo-Capable | Adds locally prefixed files such as `profile.avatar.*` with correct imports and style entry wiring. | Phase 9, Phase 16B | Keeps user naming flexible while the primitive role segment remains stable. |
...
| Area | Item | Current Maturity | Final TDD Expectation | Owner Phase | Notes |
|------|------|------------------|-----------------------|-------------|-------|
| route config | `defineRoutes(...)` | Production-Ready through 15D preloading and keepAlive integration | Provides a typed route table with path, label, page/loadPage, route refs, parent normalization, redirects, guards, preload policies, keepAlive policies, and diagnostics. | Phase 8, Phase 15A, Phase 15C, Phase 15D | Route config remains the single source of truth for routing behavior and route performance policy. |
...
| matching | `matchRoute(...)` | Production-Ready for 15A route contracts | Covers root, missing, query strings, encoded params, and deterministic matching order. | Phase 8, Phase 15A | Nested route chains and redirect leaves are covered by `matchRouteChain(...)`. |
...
| state | `provideRouter(...)` | Production-Ready through 15D preloading and keepAlive integration | Sets route table, current match, route params, cleanup, initial redirect handling, guarded initial navigation, route definition versioning, and reset cleanup for preload/keepAlive state. | Phase 8, Phase 15C, Phase 15D | Returns `Promise<boolean>` so async guard decisions can complete without a broad router event API. |
| state | `navigate(...)` | Production-Ready through 15D preloading and keepAlive integration | Handles path updates, history behavior, params, query strings, invalid route behavior, redirects, blocking guards, stale async navigation protection, and blocked keepAlive restore diagnostics. | Phase 8, Phase 15A, Phase 15C, Phase 15D | Guard/redirect navigation policy now runs through the shared decision pipeline before kept-alive views restore. |
...
| DOM | `<vr-router>` outlet | Production-Ready through 15D preloading and keepAlive integration | Renders route pages, swaps lazy pages, handles cleanup, nested outlets, error outlets, and same-day kept-alive route view detach/restore. | Phase 8, Phase 15B, Phase 15D | Nested outlets and kept-alive leaf views are covered under realistic layout transitions. |
| DOM | `<vr route.name />` link | Production-Ready through 15D preloading and keepAlive integration | Renders accessible anchors from route metadata without repeated path or label strings, routes normal clicks through guarded navigation, and preloads lazy route chains on hover/focus/touch intent. | Phase 8, Phase 15A, Phase 15C, Phase 15D | Params, query, exact active state, guard/redirect behavior, and intent preloading are covered; custom labels stay deferred. |
| diagnostics | route diagnostics foundation | Production-Ready | Provides shared `VR_ROUTE_*` codes, diagnostic factory, route-level diagnostics, param/query misuse errors, preload policy warnings, preload failures, and keepAlive diagnostics. | Phase 15A, Phase 15D | CLI/project-map diagnostics stay in a later router diagnostics slice. |
...
| metadata | primitive registry metadata | Demo-Capable through Phase 16D | Registry lists supported primitives, generated file names, asset URLs, token groups, native tags, docs paths, and future flavors. | Phase 9, Phase 16B, Phase 16D | Phase 16D adds layout, container, section, grid, stack, header, footer, sidebar, nav, breadcrumb, image, source, and `uiPrimitiveTokenGroup`. |
...
| primitive | Phase 16D layout/navigation/media primitive source templates | Demo-Capable | Each 16D primitive ships TS, HTML, CSS, usage, file-backed asset URLs, metadata, docs data, routes, and site pages. | Phase 16D | Covers layout, container, section, grid, stack, header, footer, sidebar, nav, breadcrumb, image, and source. |
| CLI | generated `src/ui/<primitive>/<prefix>.<primitive>.*` | Demo-Capable through Phase 16B | Generator handles default and local prefixes, collision safety, tokens, usage patching, style modes, and optional tests for every core primitive. | Phase 9, Phase 16B, Phase 18 | `profile.avatar.*` style is supported by local prefix. |
...
| compiler | October UI tag diagnostics | Production-Ready for Phase 16A foundation | Supported `vr-*` primitives lower through metadata and unsupported `vr-*` UI tags use October diagnostics. | Phase 16A, Phase 16B, Phase 16D | Layout/navigation/media tags are now compiler-known; form, data, and overlay semantic tags remain sliced. |
| flavor | October UI | In Progress through Phase 16D | Production primitive catalog, variants, accessibility, tokens, docs, dotted tokens, and visual QA. | Phase 16 | Phase 16D adds layout/navigation/media coverage and dogfooded docs pages; forms/data and overlays remain sliced. |
...
| generated convention | UI primitive role files | Demo-Capable through Phase 16B | UI source, template, and scoped CSS use prefix-first file names across the core primitive catalog. | Phase 9, Phase 16B | `ui.button.ts/html/css`, `profile.avatar.ts/html/css`, and equivalent primitive role triplets. |
