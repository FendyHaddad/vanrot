...
| [x] | command | `vr create <name>` | Demo-Capable through Phase 27 | Creates a routed Vanrot app, handles package managers, overwrite policy, workspace mode, clean install, optional behavior helper selection, and optional SEO selection. | Phase 5, Phase 14, Phase 16H, Phase 26, Phase 27 | Generated app includes router by default; `--behavior` and `--seo` opt into optional packages while `--no-behavior` and `--no-seo` keep the starter minimal. |
