...
| route config | `createRoutes()` | Production-Ready | Authors route refs without parent-name string literals, supports child `.page(...)` and `.layout(...)` builders, and keeps page/layout render targets separate at the TypeScript boundary. | Phase 15A, Phase 15B | Object-form `defineRoutes(...)` remains compatible. |
