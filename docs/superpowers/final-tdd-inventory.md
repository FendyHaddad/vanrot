...
| [x] | command | `vr doctor` | Demo-Capable through Phase 27 | Reports project health, Vanrot rule violations, missing scripts, raw template text, nested control-flow warnings, behavior package drift, SEO package/config drift, stable one-line findings, and structured result output. | Phase 5, Phase 14, Phase 16H, Phase 27 | Behavior checks flag unused installs and stale imports; SEO checks flag unused installs, config-without-package, imports-without-package, and config syntax diagnostics. |
