...
| DOM | `<vr-router>` outlet | Production-Ready through 15D preloading and keepAlive integration | Renders route pages, swaps lazy pages, handles cleanup, nested outlets, error outlets, same-day kept-alive route view detach/restore, router-managed internal anchor clicks, and atomic async route swaps that keep the current view visible until the next view resolves. | Phase 8, Phase 15B, Phase 15D | Nested outlets, routed internal anchors, and kept-alive leaf views are covered under realistic layout transitions. |
