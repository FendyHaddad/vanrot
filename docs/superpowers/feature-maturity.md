...
| CLI `vr create` | cli | Phase 5 | Creates a minimal Vanrot app in standalone-style mode and internal fixture/workspace mode | Package manager handling, template variants, overwrite safety, errors, and docs verified | Planned | Phase 5 owns the demo-capable create flow; production package publishing remains future work. |
| CLI `vr generate component` | cli | Phase 5 | Creates convention component files | Role suffixes, folder rules, naming diagnostics, and tests verified | Planned | Phase 5 owns demo-capable component generation. |
| CLI `vr generate page` | cli | Phase 5 | Creates convention page files | Routing integration and feature-folder placement verified | Planned | Phase 5 owns demo-capable page generation without router integration. |
| CLI `vr doctor` starter checks | cli | Phase 5 | Runs project-health checks and starter Vanrot rule checks | Diagnostics for naming, i18n, a11y, cleanup, imports, and structure verified | Planned | Phase 5 covers project health, sibling files, role suffixes, raw visible template text, and nested `if` warnings only. |
| CLI `vr dev` | cli | Phase 5 | Invokes the Vite dev server for a Vanrot app | Config loading, diagnostics, dev server lifecycle, and readable startup output verified | Planned | Thin wrapper for demo apps; production lifecycle polish remains future work. |
| CLI `vr build` | cli | Phase 5 | Invokes Vite build for a Vanrot app | Config loading, diagnostics, and build report verified | Planned | Depends on Phase 4. Build reports remain deferred. |
| CLI `vr test` | cli | Phase 5 | Invokes app tests | Testing package integration and generated test support verified | Planned | Phase 5 can use injected runner tests before `@vanrot/testing` exists. |
| CLI Quiet Premium reporter | cli | Phase 5 | Commands use a shared structured reporter with calm headings, status lines, warnings, errors, and next steps | Cross-platform terminal behavior, interactive states, theming, accessibility, snapshots, and docs verified | Planned | Phase 5 lays the output foundation; production polish comes after demo phases. |
