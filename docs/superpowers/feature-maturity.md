...
| Feature | Package or Area | Planned Phase | Demo-Capable Gate | Production-Ready Gate | Status | Notes |
|---|---|---:|---|---|---|---|
| UI package foundation | ui | Phase 9 | `@vanrot/ui` exists, builds, and exports button/tokens metadata without adding runtime weight | Public exports, package docs, tree-shaking, versioning, release checks, and ownership rules verified | Deferred | Phase 9 owns the first package shell and asset ownership boundary. |
| Design tokens | ui | Phase 9 | Generated app includes `src/styles/vanrot-tokens.css` and imports it once | Theme overrides, token documentation, dark/light policy, component integration, and visual QA verified | Deferred | Phase 9 owns a small Vanrot-branded premium token layer, not full theming. |
| Compiler-lowered `<vr-button>` | compiler and ui | Phase 9 | `<vr-button>` lowers to a native `<button>` with base class, user class passthrough, child content, events, and property bindings | Accessibility audit, form semantics, variant API, docs, diagnostics, and compatibility with future child components verified | Deferred | Phase 9 keeps clean Vanrot markup without implementing true child components. |
| CLI `vr add button` | cli and ui | Phase 9 | `vr add button` copies the default `ui.button.ts/html/css` primitive files and ensures token CSS exists | Overwrite policy, custom destinations, component registry, update workflow, docs, and multi-component support verified | Deferred | Phase 9 supports only the button primitive. |
| CLI `vr add <local-prefix> button` | cli and ui | Phase 9 | `vr add primary button` copies `primary.button.ts/html/css` into `src/ui/button/` | Naming customization docs, collision handling, aliases, feature folder support, and team conventions verified | Deferred | Local prefix is user-owned; the `.button` role segment carries the component type. |
| Typed UI primitive file roles | ui and cli | Phase 9 | Button primitives use `<local-prefix>.button.ts/html/css` | Full typed primitive role catalog, generator integration, diagnostics, docs, and AI rules verified | Deferred | Keeps `.component` for app/domain components while `.button`, `.card`, and similar segments describe reusable UI types. |
