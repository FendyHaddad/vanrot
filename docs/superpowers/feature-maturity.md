...
| Designed starter welcome page | cli, router, ui, docs | Post-demo production track | A new Vanrot app has a polished localhost page that demonstrates routes and framework feel | Visual design, accessibility, responsive behavior, route links, docs links, screenshots, and update workflow verified | Deferred | Requested as the Vanrot equivalent of the first `ng serve` impression. Keep it light and framework-owned. |
...
| Router query string API | router | Post-demo hardening | Query strings can be read without breaking path params | Typed query values, encoding, defaults, array values, history behavior, and docs verified | Deferred | Explicitly deferred from Phase 8. |
| Router nested routes and layouts | router and compiler | Post-demo hardening | Parent routes can render child outlets and layout shells | Nested cleanup, lazy boundaries, redirects, layout data, diagnostics, and docs verified | Deferred | Part of the richer router platform, kept light and future. |
| Router guards | router | Post-demo hardening | Routes can block, redirect, or allow navigation through explicit guard APIs | Async guards, cancellation, auth examples, diagnostics, testing helpers, and docs verified | Deferred | Deferred so Phase 8 stays tiny. |
| Router preloading | router and vite-plugin | Post-demo hardening | Lazy pages can be preloaded through a light strategy | Network policy, user intent heuristics, route metadata, cancellation, and metrics verified | Deferred | Future performance feature, not needed for the MVP. |
| Router active links and custom labels | router and compiler | Post-demo hardening | Route links can expose active state and override generated labels when needed | Accessibility, i18n labels, class/style integration, diagnostics, and docs verified | Deferred | Phase 8 labels live in `routes.ts`; overrides need their own design. |
| Router typed param-link generation | router and compiler | Post-demo hardening | Links to param routes can be generated without manual path strings | Type-safe params, missing-param diagnostics, encoding, template syntax, and docs verified | Deferred | Needed before templates can link to `/users/:id` without string literals. |
| Router route diagnostics | router, compiler, cli | Post-demo hardening | Invalid route refs, missing labels, duplicate paths, and unreachable routes can be reported | Code frames, docs links, strict mode, CI behavior, and route graph integration verified | Deferred | Complements Phase 7 project intelligence after the router exists. |

## `@vanrot/ui` *

*The UI package needs dedicated product/design brainstorming cycles as it matures. Phase 9 owns only the demo-capable package foundation, token CSS, `vr add button`, typed button primitive files, and compiler-lowered `<vr-button>`. The package has two planned production flavors for now: `V01`, based on the shadcn design language, and `V02`, a brutalist version of codename `V01`. These are codenames until final naming is chosen.*
...
| UI primitive catalog * | ui | Post-demo production track | Additional primitive types are planned beyond button | Button, badge, card, avatar, separator, tabs, tooltip, dropdown, popover, toast, empty-state, skeleton, progress, accessibility, docs, examples, and visual QA verified | Deferred | Requires a separate UI catalog design. Tags should follow the `vr-*` shape such as `<vr-tabs>`, `<vr-tooltip>`, and `<vr-dropdown>`. |
| UI component parameters and variants * | ui and compiler | Post-demo production track | UI components can declare basic variants without bloating runtime | Parameter syntax, type inference, default variants, accessibility states, class merging, docs, tests, and compiler diagnostics verified | Deferred | Needs dedicated design before Vanrot adds rich first-party UI APIs. |
| Tailwind interoperability | ui and docs | Post-demo production track | Vanrot UI primitives allow Tailwind classes to pass through cleanly | Setup guidance, class conflict policy, token interop, examples, docs, and build compatibility verified | Deferred | Phase 9 permits class passthrough but does not install or configure Tailwind. |
| Vanrot utility class layer * | ui | Post-demo production track | A first-party utility class set can be used alongside UI primitives | PrimeFlex-inspired naming, responsive rules, spacing/display/flex/grid coverage, docs, tree-shaking or CSS budget policy, and Tailwind coexistence verified | Deferred | Inspired by PrimeFlex-style ergonomics, but must be designed as Vanrot's own lightweight utility layer. |
| UI registry and upgrade workflow | ui and cli | Post-demo production track | Added UI primitives can be tracked for future update checks | App-owned registry, diff/update policy, lock/version metadata, conflict resolution, and docs verified | Deferred | Kept out of Phase 9 so `vr add button` stays simple. |
| UI custom add destinations | ui and cli | Post-demo production track | `vr add` can target configured folders beyond `src/ui/<type>/` | Config schema, path validation, workspace conventions, docs, and diagnostics verified | Deferred | Phase 9 writes button primitives to `src/ui/button/` only. |
